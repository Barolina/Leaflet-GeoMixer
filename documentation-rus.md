...
unbindPopup|`unbindPopup()`|`this`| Отключить обработчик баллунов по клику.
disablePopup|`disablePopup()`|`this`| Установить флаг отмены показа баллунов.
enablePopup|`enablePopup()`|`this`| Удалить флаг отмены показа баллунов.
...
setRasterHook|`setRasterHook(function(canvas, image, sx, sy, sw, sh, dx, dy, dw, dh, info): Deferred)`|`this`|Установка функции обработки растров объектов слоя. Единственный аргумент - ф-ция, аргументы функции: `canvas` - результирующий холст растра объекта, `image` - тайл растра объетка(далее источник), sx , sy, sw, sh - описание прямоугольника пикселов из источника `image`, dx, dy, dw, dh - описание прямоугольника пикселов на результирующем холсте `canvas`, info - дополнительное описание. Функция может возвращать возвращать: `Deferred` - будет произведена асинхронная обработка, `undefined` - обработка завершена в синхронном режиме.
